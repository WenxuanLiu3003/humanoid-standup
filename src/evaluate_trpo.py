...
def deterministic_action(agent: TRPO, observation: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    algo_config = load_config_or_default(
        ...
        lambda: load_algorithm_config("trpo"),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="TRPO run directory. Defaults to the latest runs/trpo/seed_* run.",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=None,
        help="Checkpoint path. Defaults to best.pt if present, otherwise final.pt.",
    )
...
def resolve_run_dir(run_dir: Path | None) -> Path:
    ...
    candidates = sorted(
        (Path("runs") / "trpo").glob("seed_*/*"),
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    if not candidates:
        raise FileNotFoundError("No TRPO runs found under runs/trpo/seed_*.")
    return candidates[0]
