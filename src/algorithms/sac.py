    ...
    def load(self, path: Path) -> None:
        ...
        if log_alpha is not None:
            self.log_alpha.data.copy_(
                torch.as_tensor(log_alpha, dtype=torch.float32, device=self.device)
            )
...
class ReplayBuffer:
    def __init__(self, *, obs_dim: int, act_dim: int, hp: dict[str, Any], device: torch.device):
        self.maxlength = int(hp.get("buffer_maxlength", 1_000_000))
        self.batchsize = int(hp.get("buffer_batch_size", 256))
        self.device = device
